...
Adds/removes a virtual disk drive and expands its size using `New-SCVirtualDiskDrive` and `Expand-SCVirtualDiskDrive`. Use `vm_id` from `data.scvmm_virtual_machine` rather than a raw name when possible. Use `move_path` to move the backing VHD with `Move-SCVirtualHardDisk`.

### `scvmm_vm_checkpoint`

Creates and removes VM checkpoints using `New-SCVMCheckpoint` and `Remove-SCVMCheckpoint`.
Prefer `vm_id` from `data.scvmm_virtual_machine` when creating checkpoints.
